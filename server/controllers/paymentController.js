...
exports.createStripeCheckout = async (req, res, next) => {
  try {
    ...
    const session = await stripeService.createCheckoutSession(
      ...
      `${clientUrl}/payment/cancel?order_id=${orderId}`
    );
...
exports.createPayPalOrder = async (req, res, next) => {
  try {
    ...
    res.json({ id: paypalOrder.id, status: paypalOrder.status, links: paypalOrder.links });
  } catch (error) {
    next(error);
  }
};
...
exports.capturePayPalOrder = async (req, res, next) => {
  try {
    ...
    res.json({ id: captureData.id, status: captureData.status, payer: captureData.payer });
  } catch (error) {
    next(error);
  }
};
